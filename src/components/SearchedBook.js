...
function SearchedBook(props) {
    ...
    const bookIsOnShelf = bookFavsCtx.bookIsOnShelf(props.data.id);

    //handlers

    function toggleAddBookHandler() {
        if (bookIsOnShelf) {
            bookFavsCtx.removeBookFromShelf(props.data.id);
            console.log(bookFavsCtx.bookshelf);
        } else {
            bookFavsCtx.addBookToShelf(props.data);
            console.log(bookFavsCtx.bookshelf);
        }
    }
                ...
                <div className="row g-0">
                    ...
                    <div className="col-md-9">
                        <div className="card-body">
                            ...
                            <button className="btn btn-primary" onClick={toggleAddBookHandler}>
                                {bookIsOnShelf ? "Remove From Shelf" : "Add book to your Bookshelf"}
                            </button>
                        </div>
                    </div>
                </div>
